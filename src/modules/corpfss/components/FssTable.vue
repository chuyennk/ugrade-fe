<template>
		...
		<vxe-modal ref="xModal" v-model="myformData.showEdit" :title="myformData.title" width="500" min-width="500" min-height="300" destroy-on-close>
			<template #default>
				<vxe-form
					title-align="top"
					title-width="100"
					:data="myformData.formData"
					:items="myformData.formItems"
					:rules="myformData.formRules"
					@submit="myformData.submitForm"
				></vxe-form>
			</template>
		</vxe-modal>
</template>
...
import FssHeader from './FssHeader.vue'
import { date } from 'quasar'
import * as math from 'mathjs'
// import { useStore } from 'vuex'
import { useCorpFssStore } from '../store'
	...
	setup(props) {
		...
		const fss_cellChanged = (row, amtYr) => {
			...
			try {
				...
				// store.setFssData(fssData.value) - This is not necessary anymore since there is no concept of immutable in pinia
			} catch (e) {
