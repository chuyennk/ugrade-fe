            ...
            <q-tab-panel name="Scard">
              <div>
                ...
                >
                  ...
                  <vxe-table-column field="scID" title="Scorecard ID" sortable width="250">
                  ...
                  <vxe-table-column field="scModelID" title="Model ID" width="80"/>
                  <vxe-table-column field="FOG" title="FOG"  width="80"/>
                  <vxe-table-column field="scStatus" title="Status"  width="80"/>
                  <vxe-table-column field="scCreatedDate" title="Created Date" :formatter="formatDate"/>
                  <vxe-table-column field="scAprrovedDate" title="Aprroved Date" :formatter="formatDate"/>
                  <vxe-table-column field="scCreatedBy" title="Created By"/>
                  <vxe-table-column field="scApprovedBy" title="Approved By"/>
                </vxe-table>
                <q-separator />
              </div>
              <div class="row q-ma-sm justify-end">
...
<script setup>
    ...
    import { useCustStore } from "../../cust/store"
    import { useCorpScStore } from "../../corpscorecard/store"
    import { useCorpFssStore } from "../../corpfss/store"

    const scStore = useCorpScStore()

    const {scList} = storeToRefs(scStore)
    const {loginInfo} = storeToRefs(useHomeStore())
    const {custInfo} = storeToRefs(useCustStore())
    const {fssData} = storeToRefs(useCorpFssStore())
    ...
    const viewScorecard = async (scID) => {
      const scorecard = await scStore.getScorecard(scID);
      if (scorecard.message == "OK") {
        await router.push("/corpsc");
      } else {
    ...
    const newScorecardDialog = async () => {
      ...
      const res = await scStore.getActScModels()
    ...
    const createNewScorecard = async () => {
      ...
      const res = await scStore.createNewScorecard(newScData)

      if (res.message=='OK'){
        const res = await scStore.getScorecardList(custInfo.value.custID);
        if (res.message=='OK'){
